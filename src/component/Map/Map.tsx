...
export default function Map({ customLocation } : { customLocation: Location }) {
    ...
    let locationBern: Location = retrievedObject ? JSON.parse(retrievedObject) : { city: "Unknown", canton: "Unknown", locality: "Unknown", lng: 0, lat: 0 }
    
    const createMap = () => {
        const map = new mapboxgl.Map({
            container: mapContainer.current,
            style: 'mapbox://styles/mapbox/streets-v11',
            center: [locationBern.lng || 0, locationBern.lat || 0],
            zoom: 9
        })

        {(customLocation.lng === 0 && customLocation.lat === 0) &&         
            new mapboxgl.Marker()
                .setLngLat([customLocation.lng, customLocation.lat])
                .addTo(map);
        }

        {(locationBern.lng !== 0 && locationBern.lat !== 0) &&        
                ...
                map.addSource('bern', {
                    ...
                    'data': {
                        ...
                        'geometry': {
                            ...
                            // [
                                    //Koordinate koje sam dobio putem API-a, koje se ne prikazuju na mapi, hadcode.
    ...
    return (
        <MapDiv ref={mapContainer} id="bern"/>
    );
}
